...
public class GameData_importer : AssetPostprocessor {
	private static readonly string filePath = "Assets/GameData.xlsx";
	private static readonly string exportPath = "Assets/GameData.asset";
	private static readonly string[] sheetNames = { "item_data", };
		...
		foreach (string asset in importedAssets) {
			...
			using (FileStream stream = File.Open (filePath, FileMode.Open, FileAccess.Read, FileShare.ReadWrite)) {
				...
				foreach(string sheetName in sheetNames) {
					...
					cell = row.GetCell(3); p.Script = (int)(cell == null ? 0 : cell.NumericCellValue);
						s.list.Add (p);
					}
					data.sheets.Add(s);
				}
